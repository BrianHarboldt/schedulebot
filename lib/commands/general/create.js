...
module.exports = new Clapp.Command({
	...
	fn: (argv, context) => {
		...
		if (time > cfg.create_channel_max_time * 60) { //bot mod can use whatever time
			...
			if (!userRole) return "Max time is 120 minutes";
		}
	...
	args: [
		...
		{
			name: "time",
			desc: "Number of minutes for the room to exist, max of 120 minutes. This can be entered as either ### for the number of minutes or #:## for the hour:minutes remaining.\nExample `!create roomname 80` or `!create roomname 1:20`",
			type: "string",
			required: true,
			validations: [
				{
					errorMessage: "Invalid time input. Example `!create roomname 80` or `!create roomname 1:20`. Max time is 120 minutes.",
