...
client.on('message', msg => {
	...
	if (msg.author.id === client.user.id)
		return; //don't talk to yourself
	
	msg.embeds.forEach(embed => {
		if (embed && embed.type === 'rich' && embed.title.indexOf('Raid') > -1)
		{
			var params = bot.BuildChannelParams(embed);
			var response = bot.CreateChannel(msg.guild, params.channelName, params.time);

			response.then(r => {
				msg.channel.send(r.msg)
					.then(m => {
						r.msg = m;
						r.channel
							.send(`https://www.google.com/maps/@${params.coords}z`)
							.then(m2 => m2.pin());
						createdRooms.push(r);
					});
			});
