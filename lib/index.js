	...
	try {
		...
		msg.embeds.forEach(embed => {
			if (embed && embed.type === 'rich' && embed.title.indexOf('Raid') > -1) {
				...
				if (roomCheck){
					console.log(`Automatic channel creation tried to make an existing channel ${roomCheck.channel} on guild ${roomCheck.channel.guild}.`);
					msg.channel.send(`Room already exists -> ${roomCheck.channel}.`); //Message origin channel about the existing channel
				} else if (minutesRemaining >= cfg.auto_create_channel_min_time) {
					...
					console.log(`Automatic room not created due to remaining time being too short - ${minutesRemaining} - ${params.channelName} on guild ${roomCheck.channel.guild}.`);
					msg.channel.send(`${params.channelName} was not created. Remaining time too short - ${minutesRemaining}.`); //Message origin channel about not making the channel
				}
			}
		});
