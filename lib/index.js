	...
	try {
		...
		msg.embeds.forEach(embed => {
			if (embed && embed.type === 'rich' && embed.title.indexOf('Raid') > -1) {
				...
				} else if (minutesRemaining >= minAutoCreateTime) { //Spawn new channel
...
client.on('ready', (event) => {
	...
	client.guilds.array().forEach(guild => {
		var categoryChannel = bot.GetCategoryChannel(guild);
		bot.SetCategoryChannel(guild, categoryChannel, client.user);
		console.log(bot.GetCategoryChannel(guild) + ` set as category channel for ${guild}`);
	});
