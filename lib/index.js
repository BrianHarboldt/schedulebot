		...
		msg.embeds.forEach(embed => {
			if (embed && embed.type === 'rich' && embed.title.indexOf('Raid') > -1) {
				...
				bot.CreateChannel(msg.guild, params.channelName, params.time)
					.then(r => { msg.channel.send(r.msg)
						.then(m => {
							//post the trigger message to the new channel
							// var clone = bot.CloneEmbed(embed);
							// r.channel.send({ embed: clone })
							// 	.then(m2 => m2.pin());

							var mapping = bot.GenerateMap(params.coords);
							var channelMap = new Discord.RichEmbed()
								.setTitle(params.channelName)
								.setURL(mapping.url)
								.setImage(mapping.map);

							r.channel.send({ embed: channelMap })
								.then(m2 => m2.pin());
...
client.on('ready', (event) => {
	...
	setInterval(function() { bot.CleanupRooms(createdRooms); }, 30000);
});
...
Promise.all([getBotTokenPromise, getBotAdminsPromise, getBlacklistPromise,
