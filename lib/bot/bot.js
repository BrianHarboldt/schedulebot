...
module.exports = {
    ...
    BuildChannelParams: function(embed) {
        ...
        var loc, poke;
        var l = /(?:\*{0,2}LOCATION:\*{0,2}\s?(.+))\n{1,}?(?:\*{0,2}ADDRESS:\*{0,2}\s?(.+))\n{1,}?/.exec(embed.description);
        if (l && l[1] && l[1].toLowerCase() != 'unknown') loc = l[1];
        else if (l && l[2] && l[2].toLowerCase() != 'unknown') loc = l[2];
        else if (parts[0].indexOf('STATS') > -1) loc = coords.replace(',', '_');
        else loc = parts[0];
        ...
        return {
            coords: coords,
            channelName: `${poke}-${loc}`.replace(/[^\w\d-_]/g, ''),
