...
let modRoles = [];
let configOverrides = []; // [{guild:Discord.Guild,modRoles:[Discord.Role],minutes:int,parser:[{type:config.MessagePart,element:config.ParserPart,value:string}],categoryChannel:Discord.Channel,user:Discord.ClientUser}]
let locations = [
    {
        ...
        roles: [
            ...
            },
            {
                role: "starbucks-fm",
                triggers: [
                    {name: "starbucks", coords: "33.041247,-97.037827", op: "OR"},
                ],
            },
        ],
    },
...
module.exports = {
    ...
    BuildChannelParams: function(guild, embed) {
        var parser = this.AutoCreateParser(guild, null);
                ...
                if (content && content.length > 1){
                    switch (section.type) {
                        ...
                        case constants.MessagePart.Time:
                            ...
                            if (preTimer)
                                time += (_raidTime * 60); //standard room time
                            break;
                        default:
                            break;
                    }
    ...
    CreateChannel: function(guild, channelName, time, coords, createdBy) {
        ...
        var categoryChannel = this.CategoryChannel(guild, null, null);
        var rawChannel;
        ...
        return rawChannel
			.then(channel => {
                ...
                var createdRoom = this.StoreChannel(channel, time, createdAt, expiresAt, coords, createdBy);
    ...
    UpdateClosingTime: function(msg, room, channelParams) { //channelParams==BuildChannelParams()
        ...
        this.StoreChannel(room.channel, room.time, room.createdAt, room.expiresAt, room.coords, room.createdBy);
        ...
        msg.channel.send(room.msgs[0]) //Message origin channel about the time being updated
    ...
    UpdateRoomName: function(guild, room, channelName) {
		if (room == null) return "Current channel not found or cannot be renamed.";

        if (channelName.indexOf('-') < 0) { //this rename is just the first part so we need to stick the second half on
			var postDashName = room.channel.name.substring(room.channel.name.indexOf('-'));
			var newChannelName = channelName + postDashName;
			console.log(`Channel rename requested with partial name. ${channelName} updated to be ${newChannelName}.`);		
			channelName = newChannelName;
        }
        
        room.channel.name = channelName;
        room.channel.setName(channelName);
        
        //Discord likes to wipe the topic out when the room is renamed; store again to be safe
        this.StoreChannel(room.channel, room.time, room.createdAt, room.expiresAt, room.coords, room.createdBy);
    ...
    AutoCreateMinTime: function(guild, minutes) {
    ...
    AutoCreateParser: function(guild, params) {
    ...
    CategoryChannel: function(guild, user, categoryChannel) {
        var config = this.GetConfig(guild);
        ...
        if (categoryChannel) {
            ...
            if (!channel) return "[Channel not found]"; //Entered room does not exist for this guild
    ...
    ModRoles: function(guild, roles) {
		...
		if (roles && roles.length > 0)
        {
            ...
            if (config)
                config.modRoles = newRoles;
            else
                configOverrides.push({guild:guild, modRoles:newRoles});
            return newRoles;
        }

        //Return just the name[] if roles were not being set
        if (config && config.modRoles)
            return config.modRoles.map(function(r){return r.name;});
        else
            return cfg.admin_app.roles.split(';');
    },
    IsInModRoles: function(guild, userRoles){
        var modRoles = this.ModRoles(guild, null);
        return userRoles.find(function(r) {
            return modRoles.find(function (mr){
                return r.name == mr;
            });            
        });
    },
    ...
    StoreChannel: function(channel, time, createdAt, expiresAt, coords, createdBy) {
