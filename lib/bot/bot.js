...
let createdRooms = []; //{msg,channel,time,createdAt,expiresAt,warningTime}
...
module.exports = {
        ...
        createdRooms.forEach(function(room, index, object) {
            if (room){
                if (room.expiresAt <= Date.now()) {
                    ...
                    room.warningTime -= 60000; //Take 1 minute until the next warning
                }
            }
        }, this);
    },
			...
			.then(channel => {
                ...
                var response = {
                    msg: `Channel created ${channel}.\n` +
                         `Expires at about ${this.ConvertTime(expiresAt)}`,
                    channel: channel,
                    time: time,
                    createdAt: createdAt,
                    expiresAt: expiresAt,
    ...
    GetClosingTime: function(guild, channelName){
        var room = this.GetRoom(guild, channelName);
        if (!room) return;
        return room.expiresAt;
    },
    GetRoom: function(guild, channelName){
