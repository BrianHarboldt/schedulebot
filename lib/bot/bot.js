...
module.exports = {
    ...
    CleanupRooms: function() {
        ...
        this.ForgetChannels(roomsToDelete);
    },
    ...
    DeleteChannel: function(guild, channelName, msg) {
        ...
        if (!createdRooms.find(function(room){ return room.channel.name == channelName || room.channel.id == channelName; }))
        {
            console.log(`Delete: tracked room not found for delete - ${channelName} on ${guild}`);
            return;
        }
        ...
        if (!channel)
        {
            console.log(`Delete: guild room not found for delete - ${channelName} on ${guild}`);
            return;
        }
        
        if (msg && typeof msg === 'object')
            msg.edit(`\Room timer expired. Removed channel ${channel.name}`);
    ...
    ForgetChannels: function(roomsToForget) {
        roomsToForget.forEach(function(room){
